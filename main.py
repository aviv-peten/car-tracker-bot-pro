...
def create_number_keyboard():
    ...
    for i in range(1, 11):
        if i == 10:
            row.append(InlineKeyboardButton("0", callback_data="num_0"))
        else:
...
async def handle_number_input(query, user_id, callback_data):
    ...
    if callback_data == "num_delete":
        ...
        if len(data['temp_car_number']) == 8:
            ...
            await query.edit_message_text(
                f"מספר רכב חייב להיות 8 ספרות (נוכחי: {len(data['temp_car_number'])})\n"
                f"מספר נוכחי: {data['temp_car_number']}\n",
                reply_markup=create_number_keyboard()
            )
            return
    else:
    ...
    if len(display_number) == 8:
        display_number = format_car_number(display_number)
